...
then
  ...
  for U in "${UTILITIES[@]}"
  do
    ALIASES_ARRAY+=("alias $U /usr/local/bin/iterm2/$U")
  done
...
FILENAME="/etc/iterm2.sh"
RELATIVE_FILENAME="${HOME_PREFIX}/.iterm2_shell_integration.${SHELL}"



test -d /usr/local/bin/iterm2 || mkdir /usr/local/bin/iterm2

for U in "${UTILITIES[@]}"
do
  echo "Downloading $U..."
  curl -SsL "https://iterm2.com/utilities/$U" > /usr/local/bin/iterm2/$U && chmod +x /usr/local/bin/iterm2/$U
done
echo "Adding aliases..."
